...
export const AGE_IN_BONDS_TITLES: Partial<Record<WithdrawalMethodKey, { title: string; subtitle: string }>> = {
  fire: {
    title: "Regra dos X% (Idade em Renda Fixa)",
    subtitle: "Acumule um múltiplo das suas despesas anuais, com alocação em renda fixa acompanhando sua idade.",
  },
  constant_withdrawal: {
    title: "Retirada constante (Idade em Renda Fixa)",
    subtitle: "Retire suas despesas atuais ajustadas pela inflação, com alocação em renda fixa acompanhando sua idade.",
  },
};

export const AGE_IN_BONDS_RATIONALE =
  "Complementado pela regra 'Idade em Renda Fixa': a alocação em RF é igual à sua idade " +
  "(ex: 40 anos = 40% RF, 60% RV). O portfólio fica progressivamente mais conservador, " +
  "reduzindo o impacto de quedas do mercado conforme você envelhece.";

export const AGE_IN_BONDS_PROS: ProConItem[] = [
  { text: "Alocação se ajusta automaticamente ao risco apropriado para a idade", ageInBonds: true },
  { text: "Reduz volatilidade progressivamente — protege contra quedas do mercado", ageInBonds: true },
];

export const AGE_IN_BONDS_CONS: ProConItem[] = [
  { text: "Alta alocação em RF em idades avançadas pode reduzir crescimento do portfólio", ageInBonds: true },
  { text: "Requer rebalanceamento anual entre RF e RV", ageInBonds: true },
];
