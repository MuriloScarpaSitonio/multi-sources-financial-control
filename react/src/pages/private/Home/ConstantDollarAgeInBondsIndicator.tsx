...
const computeProjection = (
  ...
  bondReturn: number,
  targetYears: number,
): { points: ProjectionPoint[]; depletionYears: number } => {
  const points: ProjectionPoint[] = [];
  let balance = portfolio;
  let expenses = annualExpenses;
  let depletionYears = targetYears;

  for (let i = 0; i <= targetYears; i++) {
    const age = currentAge + i;
    const bondPct = Math.min(age, 100);
    const stockPct = 100 - bondPct;
    const blendedReturn = (bondPct * bondReturn + stockPct * stockReturn) / 100 / 100;

    points.push({
      age,
      withdrawal: balance > 0 ? expenses / 12 : 0,
      portfolio: Math.max(balance, 0),
      bondPct,
    });

    if (balance > 0) {
      balance = balance * (1 + blendedReturn) - expenses;
      if (balance <= 0 && depletionYears === targetYears) {
        depletionYears = i + 1;
      }
    }
    expenses *= 1 + inflation / 100;
  }
  return { points, depletionYears };
};
...
const ConstantDollarAgeInBondsIndicator = ({
  ...
  const { points: projection, depletionYears } = useMemo(() => {
    if (currentAge === null || annualExpenses <= 0)
      return { points: [], depletionYears: 0 };
  ...
  }, [effectivePatrimony, annualExpenses, currentAge, inflation, stockReturn, bondReturn, targetYears]);
  ...
  const depletionLabel = depletionYears >= targetYears ? `${targetYears}+` : depletionYears;
  const monthlyFormatted = hideValues ? "***" : formatCurrency(avgExpenses);
  const tooltipTitle =
    `Retirada constante com alocação Idade em Renda Fixa. ` +
    `Retire suas despesas atuais (${monthlyFormatted}/mês) ajustadas pela inflação (${inflation}% a.a.). ` +
    `Retornos mistos: RV ${stockReturn}%, RF ${bondReturn}%, alocação RF = idade. ` +
    `Estimativa: ${depletionLabel} anos. Meta: ${targetYears} anos.`;
