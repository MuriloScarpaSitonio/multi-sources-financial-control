...
class KuCoinClient:
    ...
    def __init__(self, user: User, timeout: int = 30) -> None:
        self._api_secret_key = decrypt(user.secrets.kucoin_api_secret)
        self._session = ClientSession(
            ...
            headers={
                ...
                "KC-API-KEY": decrypt(user.secrets.kucoin_api_key),
                "KC-API-PASSPHRASE": decrypt(user.secrets.kucoin_api_passphrase),
                "timeout": str(timeout),
            },
        )
    ...
    async def _create_path(self, path: str, api_version: Optional[str] = None) -> str:
