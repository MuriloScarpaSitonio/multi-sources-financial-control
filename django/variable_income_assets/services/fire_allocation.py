...
def classify_return_bucket(
    *, asset_type: str, indexer: str, maturity_date: date | None, today: date
    ...
    if indexer == FixedIncomeIndexers.prefixed:
        series = (
            "IRF_M_1"
            if maturity_date <= today + relativedelta(years=1)
            else "IRF_M_1_PLUS"
        )
        return "FIXED_PREFIXED", series
    series = (
        "IMA_B_5" if maturity_date <= today + relativedelta(years=5) else "IMA_B_5_PLUS"
    )
    return "FIXED_IPCA", series


def build_fire_allocation(
    *, user_id: int, today: date | None = None
) -> list[FireAllocationBucket]:
    ...
    for asset in assets:
        ...
        totals[bucket] = totals.get(bucket, Decimal()) + asset.normalized_current_total
    ...
    return [
        FireAllocationBucket(category=category, series=series, total=total)
        for (category, series), total in sorted(
            totals.items(), key=lambda item: order[item[0][0]]
        )
        if total
    ]
